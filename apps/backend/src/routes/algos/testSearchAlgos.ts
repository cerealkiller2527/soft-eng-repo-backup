...
function findNode(startNode: Nodef, endNode: Nodef, path: Nodef[]): Nodef[] {
    ...
    if(startNode.id === endNode.id) { // base case: found the path
        console.log("Found a path!");
        printPath(newPath);
        return newPath;
    }else { // if we have not found the path
...
function findBFS(startNode: Nodef, endNode: Nodef) {
    ...
    while (pathQueue.length > 0) {
        ...
        if (currentNode === endNode) { // if this node is the end node
            console.log("Found a path!");
            return currentPath;
        }

        if (!visited.has(currentNode)) { // if this node is unvisited
